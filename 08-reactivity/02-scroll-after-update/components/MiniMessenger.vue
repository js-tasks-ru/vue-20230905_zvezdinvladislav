<template>
  <main class="mini-messenger">
    <ul class="messages">
      <li ref="targetRef" v-for="message in messages" :key="message.id" class="message">
        {{ message.text }}
      </li>
    </ul>
...
export default {
  ...
  methods: {
    handleSendSubmit() {
      ...
      nextTick(() => {
        let refsArrLength = this.$refs.targetRef.length;
        this.$refs.targetRef[refsArrLength - 1].scrollIntoView();
      });
    },
